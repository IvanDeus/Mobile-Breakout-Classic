...
export class LevelService {
  ...
  getLevelLayout(lvl: number): BrickPosition[] {
    ...
    switch (lvl) {
      ...
      case 3: {
        const cx = 4.5, cy = 3;
        for (let r = 0; r < 7; r++) for (let c = 0; c < maxC; c++) if (Math.abs(r - cy) + Math.abs(c - cx) <= 3.5) positions.push({ r, c });
        break;
      }
      case 4:
        ...
        break;
      case 5:
        for (let r = 0; r < 6; r++) for (let c = 0; c < maxC; c++) if (r === 0 || r === 5 || c === 0 || c === 9) positions.push({ r, c });
        break;
      case 6:
        for (let r = 0; r < 7; r++) for (let c = 0; c < maxC; c++) if ((c >= 3 && c <= 6) || (r >= 2 && r <= 4)) positions.push({ r, c });
        break;
      case 7:
        for (let r = 0; r < 5; r++) {
          const l = r, ri = maxC - 1 - r;
          positions.push({ r, c: l });
          if (l + 1 <= 4) positions.push({ r, c: l + 1 });
          if (ri !== l) { positions.push({ r, c: ri }); if (ri - 1 >= 5) positions.push({ r, c: ri - 1 }); }
        }
        break;
      case 8:
        for (let r = 0; r < 5; r++) for (let c = 0; c < maxC; c++) if (c < 4 || c > 5) positions.push({ r, c });
        break;
      case 9:
        for (let r = 0; r < 6; r++) { const o = (r % 2 === 0) ? 0 : 2; for (let c = o; c < o + 7 && c < maxC; c++) positions.push({ r, c }); }
        break;
      case 10:
